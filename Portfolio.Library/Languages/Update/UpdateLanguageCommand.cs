...
public record UpdateLanguageCommand(string Id, string Name, int Proficiency) : IRequest<bool>;
...
public class UpdateLanguageCommandHandler : IRequestHandler<UpdateLanguageCommand, bool>
{
    ...
    public async Task<bool> Handle(UpdateLanguageCommand request, CancellationToken cancellationToken)
    {
        ...
        var index = languages.FindIndex(e => e.Id == request.Id);
        if (index == -1)
            return false;
            
        languages[index] = new Domain.Language
        {
            Id = request.Id,
            Name = request.Name,
